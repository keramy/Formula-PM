...
class AutoReportService {
  ...
  /**
   * Generate complete report from photo selection with one click
   */
  async generateFromPhotos(projectId, photoSelection, options = {}) {
    try {
      ...
      // Get photo data
      const photos = await this.resolvePhotoData(photoSelection);
      
      // Apply filters if specified
      const filteredPhotos = this.applyPhotoFilters(photos, options.filters);
      
      // Generate smart report structure
      const smartReport = await smartTemplateService.generateSmartReport(
        projectId, 
        filteredPhotos, 
      ...
      const report = await reportService.createReport({
        projectId,
        title: smartReport.reportStructure.title,
        type: smartReport.reportStructure.type,
        sections: smartReport.reportStructure.sections,
        metadata: {
          ...smartReport.reportStructure.metadata,
          autoGenerated: true,
      ...
      const generationRecord = {
        ...
        templateUsed: smartReport.template.id,
        generatedAt: new Date().toISOString(),
        generationTime: Date.now() - startTime,
        options: options,
        analysisResult: smartReport.analysisResult
      };
      ...
      return {
        ...
        generationInfo: {
          ...generationRecord,
          confidence: smartReport.analysisResult.confidence
        },
        recommendations: this.generateRecommendations(smartReport.analysisResult, filteredPhotos)
      };
